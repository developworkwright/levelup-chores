...
class ChoreIconTest extends TestCase
{
    ...
    {
        // A key with no path behind it renders nothing at all, which on a card
        // is a blank face — the one outcome the fallback chain exists to stop.
        foreach (ChoreIcon::cases() as $icon) {
            $rendered = view('components.chore-icon', ['icon' => $icon, 'class' => 'h-6 w-6'])->render();

            $this->assertStringContainsString('<svg', $rendered, "{$icon->value} renders nothing.");
            $this->assertStringContainsString('stroke="currentColor"', $rendered);
            // A fill would stop the icon tracking the text colour and go black
            // on the dark panel.
            $this->assertStringContainsString('fill="none"', $rendered);
        }
    }
    ...
    public function test_a_new_chore_gets_a_face_guessed_from_its_name(): void
    {
        ...
        $this->assertSame(ChoreIcon::Lawn, Chore::firstWhere('name', 'Mow the lawn')->icon);
    }
    ...
    public function test_a_parent_can_set_and_clear_a_chores_face(): void
    {
        ...
        $this->assertSame(ChoreIcon::Toys, $chore->fresh()->icon);
