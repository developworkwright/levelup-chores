...
/**
 * Where a playlist gets built — the same component in both consoles.
 *
 * The split it sits on either side of is the whole design of this. Choosing and
 * playing music happens in the header, on whatever page you are already on,
 ...
 * dropdown on a phone.
 *
 * One component for both consoles because a playlist is a playlist: the table
 * is keyed to a profile rather than to a kid (see the migration), and every
 * rule about names, limits and songs that have gone missing is the same
 * whoever made the list. Only the wording and the card it sits in differ, and
 * those come off `audience` — the same shape as push-toggle, for the same
 * reason.
 *
...
{
    /** 'kid' — its own page under Me. 'parent' — a section under the library. */
    #[Locked]
    ...
    public function mount(string $audience = 'kid'): void
    {
        ...
        // Straight into the only one they have. Somebody with a single playlist
     ...
     * hard way — there is a test that keeps both of them honest.
     */
    ...
    /**
     * Redraw when the library itself changed under us.
     *
     * Only ever fires in the parent console, where the screen that uploads,
     * renames and deletes songs is the same screen this sits on: a song
     * removed above would otherwise still be offered down here, and adding it
     * would fail with nothing to say for itself.
     */
    #[On('music-library-changed')]
    ...
    public function createPlaylist(): void
    {
        ...
        $this->errorMessage = $service->rejection($profile, $this->newName);

        if ($this->errorMessage !== null) {
            return;
        }

        $playlist = $service->create($profile, $this->newName);
    ...
    public function renamePlaylist(int $id, string $name): void
    {
        ...
        // what was typed, so a rename that just didn't happen looks like it did
    ...
    /**
     * Whoever is signed in, off the guard rather than out of a property.
     *
     * A nested component is handed its parameters by the page that draws it,
     * and a profile that arrived that way is one the browser gets to send back.
     * The lists are per profile precisely so nobody edits anybody else's.
     */
    ...
    /**
     * One of this profile's playlists, by id, or null.
     *
     * Every method above goes through here — see profile() for why the owner is
    ...
    public function with(): array
    {
        ...
        return [
            ...
            'forParent' => $this->audience === 'parent',
        ];
    }
}; ?>

<div class="flex flex-col {{ $forParent ? 'gap-3' : 'gap-[14px]' }}">
    {{-- The heading and the box it sits in are the only things the two consoles
         do differently: a kid's music page opens with it, a parent's is a
         second section under the library they just added songs to. --}}
    <div
        class="{{ $forParent ? 'rounded-[28px] border border-fq-line bg-fq-bg p-[16px_14px]' : 'rounded-[24px] border p-6' }}"
        @style(['background: var(--fq-wash-blue); border-color: var(--fq-line-cool)' => ! $forParent])
    >
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <h2 class="{{ $forParent ? 'font-baloo text-xl font-extrabold' : 'mt-1 font-baloo text-2xl font-extrabold' }}">
                    {{ $forParent ? 'Your playlists' : 'Music' }}
                </h2>
            </div>
        ...
        <p class="{{ $forParent ? 'mt-[3px] text-xs text-fq-text-3' : 'mt-3 text-[13px] text-fq-text-3' }}">
            ...
            {{-- Said out loud on the console that administers everything else in
                 the house: a parent reading this should not have to wonder
                 whether they are editing one of the kids' lists. --}}
            @if ($forParent)
                These are yours alone — the kids keep their own.
            @endif
        </p>
        ...
        @if ($librarySize === 0)
            <p class="mt-3 text-[13px] text-fq-text-5">
                @if ($forParent)
                    There is no music in the house yet — add a song up above.
                @else
    ...
    @if ($playlists->isEmpty())
        {{-- Only on the kid page. On the parent screen the card above is one
             section of several, and a second empty-state box under it says the
             same thing twice on a page that is mostly about the library. --}}
        @if ($librarySize > 0 && ! $forParent)
            ...
            @foreach ($playlists as $playlist)
                <div wire:key="playlist-{{ $playlist->id }}" class="flex flex-col gap-2">
                    <div class="flex flex-wrap items-center gap-2 rounded-[18px] border border-fq-line-2 bg-fq-panel px-3 py-[11px]">
                        ...
                        {{-- The name is the input. Renaming a playlist is the
                             only thing anybody does to one that isn't about its
                             songs, so it doesn't need a mode. --}}
                        <input
                            type="text"
                            value="{{ $playlist->name }}"
                            maxlength="{{ $maxName }}"
                        ...
                        {{-- Playing is the header's job everywhere else in the
                             app, and it is the header's job here too: this
                             reaches straight into the same store, so there is
                             only ever one thing making noise. --}}
                        <button
                            type="button"
                            x-data
                        ...
                        <div class="flex flex-col gap-[6px] pl-4">
                            @forelse ($songs as $index => $song)
                                ...
                                >
                                    ...
                                    {{-- A song whose file has gone. Shown rather
                                         than quietly dropped: a list that
                                         shrinks on its own looks like the app
                                         losing things. --}}
                                    @if ($missing)
    ...
    {{-- The library, and only once a list is open to put things in. An add
         button with nowhere to add to is a button that has to explain itself,
         and this page would be mostly that. --}}
    @if ($open !== null)
